...
static char RegisterText[LastReg][32] = { ",,,", "B",  "DT", "F",  "HF", "I",  "K",	 "LF", "R",
...
static void DecodeFile( char *FileName, InstRecord **Head, SymbolRecord *Dir, SymbolRecord **Symb,
						SymbolRecord **Cond );
...
static void ListInstruction( long int Address, unsigned int Count, InstRecord *Inst )
{
	...
	if( fprintf( ListFile, "%s\n%s(%u).. %s: ", Inst->Loc.Text, Inst->Loc.Name, Inst->Loc.Line,
				 number_to_string( buffer, Address, 16, 3 ) ) <= 0 )
		RunError( True, FileWriteError );
...
{
	DefineSymbol( (char *)"SCHIP10", UnDefined, &Conditions, &NullLocation );
	Super10Cond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"CHIP48", Defined, &Conditions, &NullLocation );
	Chip48Cond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"USEDYES", UnDefined, &Conditions, &NullLocation );
	UsedYesCond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"ALIGNON", Defined, &Conditions, &NullLocation );
	AlignOnCond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"HPASC", UnDefined, &Conditions, &NullLocation );
	HpAscCond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"SCHIP11", UnDefined, &Conditions, &NullLocation );
	Super11Cond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"XREFON", Defined, &Conditions, &NullLocation );
	XrefOnCond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"CHIP8", UnDefined, &Conditions, &NullLocation );
	Chip8Cond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"HPHEAD", Defined, &Conditions, &NullLocation );
	HpHeadCond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"USEDON", UnDefined, &Conditions, &NullLocation );
	UsedOnCond = &( CurrentSymbol->Value );
	DefineSymbol( (char *)"XREFYES", Defined, &Conditions, &NullLocation );
...
static char ResolveBinaryOperand( char Token, long int *Value, long int Operand )
{
	...
	case PowerChar: {
		...
		while( Count++ <= Operand ) This *= *Value;
...
static void DecodeDirective( unsigned int Token, char **Line, InstRecord **Head, SymbolRecord *Dir, SymbolRecord **Symb,
							 SymbolRecord **Cond )
{
	ParamRecord *Params;
	InstRecord *Inst;
...
static void EncodeValToken( InstRecord *Inst, long int OpCode, long int Limit, SymbolRecord *Symb, long int *Ver8,
							long int *Ver11 )
{
	long int Value;
...
static void EncodeRegRegOrValToken( InstRecord *Inst, long int OpCode1, long int OpCode2, SymbolRecord *Reg,
									SymbolRecord *Symb )
{
	long int RegX;
	long int RegY;
...
static void WriteCheckByte( long int *Check, unsigned char Value )
{
	...
	if( *HpHeadCond ) {
		if( fprintf( OutFile, "%s%s", number_to_string( LsbText, (long int)( Value & NibbleMask ), 16, 1 ),
					 number_to_string( MsbText, (long int)( ( Value >> 4 ) & NibbleMask ), 16, 1 ) ) <= 0 )
			RunError( True, FileWriteError );
		...
		if( *HpHeadCond ) {
			for( Count = 0; Count < 4; Count++ )
				if( fprintf( OutFile, "%s",
							 number_to_string( HexText, ( Check >> ( 4 * Count ) ) & NibbleMask, 16, 1 ) ) <= 0 )
					RunError( True, FileWriteError );
...
{
	static char valid_operators[] = { StartChar, StopChar, PlusChar, MinusChar, NotChar, PowerChar, ShlChar, ShrChar,
									  MulChar,	 FracChar, AndChar,	 OrChar,	XorChar, DivChar,	ModChar, '\0' };

	for( unsigned int idx = 0x0; idx < 0x100; idx++ ) Operators[idx] = False;

	for( unsigned int idx = 0x0; valid_operators[idx]; idx++ ) Operators[idx] = True;
}
