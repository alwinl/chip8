...
static SymbolString TokenText[LastToken] = {
	",,,,,", "=",	   "ADD", "ALIGN", "AND",  "CALL", "CLS",	"DA",	 "DB",	  "DEFINE",	 "DRW", "DS",
	"DW",	 "ELSE",   "END", "ENDIF", "EQU",  "EXIT", "HIGH",	"IFDEF", "IFUND", "INCLUDE", "JP",	"LD",
	"LOW",	 "OPTION", "OR",  "ORG",   "RET",  "RND",  "SCD",	"SCL",	 "SCR",	  "SE",		 "SHL", "SHR",
	"SKNP",	 "SKP",	   "SNE", "SUB",   "SUBN", "SYS",  "UNDEF", "USED",	 "XOR",	  "XREF" };

static SymbolString RegisterText[LastReg] = { ",,,", "B",  "DT", "F",  "HF", "I",  "K",	 "LF", "R",
											  "ST",	 "V0", "V1", "V2", "V3", "V4", "V5", "V6", "V7",
...
static char ResolveDivMod( char *Symbol, char **Param, long int *Value, SymbolPointer Head, LocPointer Loc );
static void DecodeFile( char *FileName, InstPointer *Head, SymbolPointer Dir, SymbolPointer *Symb,
						SymbolPointer *Cond );

static void RunError( char AbortFlag, char *Message )
{
	if( AbortFlag ) {
		fprintf( stderr, "%c%s\n%s%s\n", BellChar, Separator, RunErrorMessage, Message );
		if( ListFile )
	...
	if( SymbPoint ) {
		fprintf( stderr, "Symbol %s file %s line %u\n%s\n", SymbPoint->Symbol, SymbPoint->Loc.Name, SymbPoint->Loc.Line,
				 SymbPoint->Loc.Text );
		if( ListFile )
			fprintf( ListFile, "Symbol %s file %s line %u\n%s\n", SymbPoint->Symbol, SymbPoint->Loc.Name,
					 SymbPoint->Loc.Line, SymbPoint->Loc.Text );
	}
	...
	if( AbortFlag )
		exit( FailExitCode ); /* exit function should close all files */
	else
		WarningCount++;
}

static char *NumberString( char *Result, long int Value, unsigned int Base, unsigned int Count )
{
	char *Digit;

	Digit = Result + Count; // right justify
	*( Digit-- ) = NullChar;
	...
	while( Digit >= Result ) *( Digit-- ) = '0'; // zero padding at front

	return ( Result );
}

static void ListInstruction( long int Address, unsigned int Count, InstPointer Inst )
{
	unsigned int This;
	SymbolString HexText;

	if( fprintf( ListFile, "%s\n%s(%u).. %s: ", Inst->Loc.Text, Inst->Loc.Name, Inst->Loc.Line,
				 NumberString( HexText, Address, 16, 3 ) ) <= 0 )
		RunError( True, FileWriteError );

	for( This = 0; This < Count; This++ )
		if( fprintf( ListFile, NumberString( HexText, (long int)Memory[Address + This - StartAddress], 16, 2 ) ) <= 0 )
			RunError( True, FileWriteError );

	fprintf( ListFile, "\n" );
}

static void ListReference( ReferPointer Head )
{
...
static void ListSymbols( SymbolPointer Head )
{
	SymbolString HexText;
	...
	if( Head ) {
		...
		if( fprintf( ListFile, "%s %s %s(%u)\n", HexText, Head->Symbol, Head->Loc.Name, Head->Loc.Line ) <= 0 )
			RunError( True, FileWriteError );
...
static void ListDefines( SymbolPointer Head )
{
	if( Head ) {
		if( Head->Left )
		...
		} else if( fprintf( ListFile, "UND %s\n", Head->Symbol ) <= 0 )
			RunError( True, FileWriteError );
...
static void ClaimSpace( SpaceRecord *Chain, unsigned int Size )
{
	...
	if( ( Chain->Request == 0 ) || ( Size > Chain->Request ) )
		RunError( True, SpaceAssertError );
...
static char OpenFiles( int argc, char *argv[] )
{
	...
	fprintf( stderr, "\n" );
	if( fprintf( ListFile, "%s\n\nTargetFile: %s\nSourceFile: %s\nListFile: %s\n\n", CopyRight, OutFileName, InFileName,
				 ListFileName ) <= 0 )
		RunError( True, FileWriteError );
...
static void DefineReference( LocPointer Loc, ReferPointer *Head )
{
	ReferPointer This;
	ReferPointer Ref;
	ReferPointer Last;
	...
	if( Loc ) {
		...
		if( !Last )
			*Head = This;
		else if( ( Loc->Line != Last->Loc.Line ) || ( Loc->Name != Last->Loc.Name ) )
			Last->Next = This;
	}
}

static char *StripSymbol( char *Result, char *Symbol )
{
...
static char DefineSymbol( char *RawSymbol, long int Value, SymbolPointer *Head, LocPointer Loc )
{
	SymbolString Symbol;
	SymbolPointer Symb;
	SymbolPointer Last;
	...
	while( Symb ) {
		...
		Symb = ( Compare <= 0 ) ? Symb->Left : Symb->Right;
	}
...
static char ResolveOption( char *Option, long int *YesNoPoint, long int *OnOffPoint )
{
	char Result;
...
static char ResolveSymbol( char *RawSymbol, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	SymbolString Symbol;
	int Compare;
	char Result;
...
static char ResolveNumber( char *Symbol, int Base, long int *Value )
{
	char *Digit;
	char Result;
	unsigned int Count;
...
static char ResolveValue( char *Symbol, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	char Result;
	...
	*Value = 0L;
	...
	switch( Symbol[0] ) {
	case HexChar: Result = ResolveNumber( Symbol + 1, 16, Value ); break;
	case BinChar: Result = ResolveNumber( Symbol + 1, 2, Value ); break;
	case OctChar: Result = ResolveNumber( Symbol + 1, 8, Value ); break;
	case AscChar:
		if( Symbol[1] ) {
			*Value = Symbol[1];
			if( !( Symbol[2] ) )
...
static char *SplitParam( char *Result, char **Param )
{
	char *Start;
	char *Count;
	char *Store;
...
static char ResolveOperator( char *Symbol, char *Token, char *Legal )
{
	*Token = *Symbol;
...
static char ResolveSingle( char Token, long int *Value )
{
	char Result;
	...
	case PlusChar:
		/* Nothing to be done */
		break;
	case MinusChar: *Value = -( *Value ); break;
	case NotChar: *Value = ~( *Value ); break;
	default: Result = False; break;
	}
	return ( Result );
}

static char ResolveDouble( char Token, long int *Value, long int Operand )
{
	long int Count;
	long int This;
	char Result;
	...
	case PowerChar:
		This = 1;
		for( Count = 1; Count <= Operand; Count++ ) This *= *Value;
		*Value = This;
		break;
	case ShlChar: *Value <<= Operand; break;
	case ShrChar: *Value >>= Operand; break;
	case MulChar: *Value *= Operand; break;
	case FracChar:
	case DivChar: *Value /= Operand; break;
	case PlusChar: *Value += Operand; break;
	case MinusChar: *Value -= Operand; break;
	case AndChar: *Value &= Operand; break;
	case OrChar: *Value |= Operand; break;
	case XorChar: *Value ^= Operand; break;
	case ModChar: *Value %= Operand; break;
	default: Result = False; break;
	}
	return ( Result );
}

static char ResolveParent( char *Symbol, char **Param, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	char Token;
	char Status;
...
static char ResolveUnary( char *Symbol, char **Param, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	char Token;
	char Status;
...
static char ResolvePower( char *Symbol, char **Param, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	long int Operand;
	char Token;
	char Status;
...
static char ResolveMulDiv( char *Symbol, char **Param, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	long int Operand;
	char Token;
	char Status;
...
static char ResolvePlusMinus( char *Symbol, char **Param, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	long int Operand;
	char Token;
	char Status;
...
static char ResolveBitWise( char *Symbol, char **Param, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	long int Operand;
	char Token;
	char Status;
...
static char ResolveExpression( char **Param, long int *Value, SymbolPointer Head, LocPointer Loc )
{
	SymbolString Symbol;
	char Result;
...
static void MissingEquations( SymbolPointer Head )
{
	if( Head ) {
...
static unsigned int ResolveTraversal( SymbolPointer Symb, SymbolPointer Head )
{
	char *Expression;
	long int Value;
	unsigned int Count;
...
static void ResolveEquations( SymbolPointer Head )
{
	unsigned int Count;
...
static void StoreSymbolList( SymbolString SymbolText[], unsigned int Min, unsigned int Max, SymbolPointer *DirHead )
{
	unsigned int This;

	This = Min + ( ( Max - Min ) >> 1 );
	if( !DefineSymbol( SymbolText[This], (long int)This, DirHead, &NullLocation ) )
...
static char *SplitLine( char *Result, char **Line, char AbortFlag )
{
	...
	*Store = NullChar;
	...
	for( Start = NULL, Count = *Line; *Count; Count++ )
		if( *Count == RemarkChar )
			Count = NullString;
		else if( isgraph( *Count ) && ( *Count != SeparatorChar ) ) {
			Start = Count;
			Count = NullString;
		}
	if( Start )
...
static char ParamCheck( unsigned int Count, unsigned int Min, unsigned int Max )
{
...
static char RangeCheck( long int Value, long int Min, long int Max, char *Message )
{
...
static unsigned int DecodeParameters( char **Line, ParamPointer *Head )
{
...
static void DecodeDirective( unsigned int Token, char **Line, InstPointer *Head, SymbolPointer Dir, SymbolPointer *Symb,
							 SymbolPointer *Cond )
{
	ParamPointer Params;
	InstPointer Inst;
	...
	default:
		if( *StackPoint ) {
			switch( Token ) {
			case WrongToken: RunError( False, InternalWarning ); break;
			case AlignToken:
				if( ParamCheck( Count, 1, 1 ) )
			...
			case UndefToken:
				if( ParamCheck( Count, 1, 1 ) )
					if( Token == DefineToken )
						...
						DefineSymbol( Params->Param, UnDefined, Cond, &NullLocation );
				break;
			case EqualToken:
			case EquToken:
				if( !CurrentSymbol )
					...
					Expression = (char *)AssertSpace( &Space, sizeof( ParamString ) );
					CurrentSymbol->Expr = strcpy( Expression, Params->Param );
					ClaimSpace( &Space, (unsigned int)( strlen( Expression ) + 1 ) );
				}
				break;
			case DsToken:
				if( ParamCheck( Count, 1, 1 ) ) {
			...
			case IncludeToken:
				if( ParamCheck( Count, 1, 1 ) ) {
					...
					IncludeName = (char *)AssertSpace( &Space, sizeof( LineString ) );
...
static void DecodeLine( char **Line, InstPointer *Head, SymbolPointer Dir, SymbolPointer *Symb, SymbolPointer *Cond )
{
	ParamString Split;
	long int Value;
			...
			if( !ResolveSymbol( Split, &Value, Dir, &NullLocation ) ) {
				if( *StackPoint )
					if( CurrentSymbol ) {
						...
						*Line = NullString;
					} else if( !DefineSymbol( Split, Location.Addr, Symb, &Location ) )
						RunError( False, CopySymbolWarning );
			} else {
...
static void DecodeFile( char *FileName, InstPointer *Head, SymbolPointer Dir, SymbolPointer *Symb, SymbolPointer *Cond )
{
	char *StoreStack;
	char *Line;
...
static void EncodeNoneToken( InstPointer Inst, long int OpCode, long int *Ver10, long int *Ver11 )
{
...
static void EncodeValToken( InstPointer Inst, long int OpCode, long int Limit, SymbolPointer Symb, long int *Ver8,
							long int *Ver11 )
{
	long int Value;
...
static void EncodeRegToken( InstPointer Inst, long int OpCode, SymbolPointer Reg )
{
	long int RegX;
	...
	if( RangeCheck( RegX, 0L, NibbleMask, BadRegisterWarning ) )
		RegX = 0L;
	RegX = OpCode | ( RegX << 8 );
	Memory[Inst->Loc.Addr - StartAddress] = (unsigned char)( ( RegX & 0xff00L ) >> 8 );
	Memory[Inst->Loc.Addr - StartAddress + 1] = (unsigned char)( RegX & 0xffL );
	ListInstruction( Inst->Loc.Addr, 2, Inst );
}

static void EncodeRegValToken( InstPointer Inst, long int OpCode, SymbolPointer Reg, SymbolPointer Symb )
{
	long int RegX;
	long int Value;
...
static void EncodeRegRegToken( InstPointer Inst, long int OpCode, unsigned int Min, SymbolPointer Reg )
{
	long int RegX;
	long int RegY;
...
static void EncodeRegRegOrValToken( InstPointer Inst, long int OpCode1, long int OpCode2, SymbolPointer Reg,
									SymbolPointer Symb )
{
	long int RegX;
	long int RegY;
...
static void EncodeDrwToken( InstPointer Inst, long int OpCode, SymbolPointer Reg, SymbolPointer Symb )
{
	long int RegX;
	long int RegY;
	long int Value;
...
static void EncodeAddToken( InstPointer Inst, SymbolPointer Reg, SymbolPointer Symb )
{
	long int RegX;
	long int RegY;
	long int Value;
...
static void EncodeJpToken( InstPointer Inst, SymbolPointer Reg, SymbolPointer Symb )
{
	long int RegX;
	long int Addr;
	...
	if( ParamCheck( Inst->Count, 1, 2 ) )
		if( ResolveSymbol( Inst->Params->Param, &RegX, Reg, &NullLocation ) ) {
			...
			if( RangeCheck( Addr, 0L, AddrMask, RangeWarning ) )
				Addr &= AddrMask;
			Addr |= 0xb000L;
			Memory[Inst->Loc.Addr - StartAddress] = (unsigned char)( ( Addr & 0xff00L ) >> 8 );
			Memory[Inst->Loc.Addr - StartAddress + 1] = (unsigned char)( Addr & 0xffL );
			ListInstruction( Inst->Loc.Addr, 2, Inst );
		} else {
			char *FirstParam;
...
static void EncodeLdToken( InstPointer Inst, SymbolPointer Reg, SymbolPointer Symb )
{
	long int RegX;
	long int RegY;
	long int Value;
	...
	case IiReg:
		...
		switch( RegY ) {
		case BReg: Value = 0xf033L | ( RegX << 8 ); break;
		case DtReg: Value = 0xf015L | ( RegX << 8 ); break;
		case FReg:
			if( !*Chip48Cond )
		...
		case RReg:
			...
			break;
		case StReg: Value = 0xf018L | ( RegX << 8 ); break;
		case IiReg: Value = 0xf055L | ( RegX << 8 ); break;
		default: RunError( False, InternalWarning ); break;
		}
		Memory[Inst->Loc.Addr - StartAddress] = (unsigned char)( ( Value & 0xff00L ) >> 8 );
		Memory[Inst->Loc.Addr - StartAddress + 1] = (unsigned char)( Value & 0xffL );
		ListInstruction( Inst->Loc.Addr, 2, Inst );
...
static void EncodeDaToken( InstPointer Inst )
{
	char *Param;
	char *This;
	long int Store;
	...
	for( This = Param; *This; This++ ) Memory[Store++] = *This;

	ListInstruction( Inst->Loc.Addr, (unsigned int)strlen( Param ), Inst );
}

static void EncodeDbToken( InstPointer Inst, SymbolPointer Symb )
{
	ParamPointer Param;
	char *LoopParam;
	long int Value;
...
static void EncodeDwToken( InstPointer Inst, SymbolPointer Symb )
{
	ParamPointer Param;
	char *LoopParam;
	long int Value;
...
static void EncodeInstruction( InstPointer Inst, SymbolPointer Reg, SymbolPointer Symb )
{
	switch( Inst->Token ) {
...
static void EncodeMemory( InstPointer Inst, SymbolPointer Reg, SymbolPointer Symb, SymbolPointer Cond )
{
	long int Count;
	...
	for( Count = StartAddress; Count <= StopAddress; Count++ ) Memory[Count - StartAddress] = 0;
...
static void WriteCheckByte( long int *Check, unsigned char Value )
{
	...
	if( *HpHeadCond ) {
		if( fprintf( OutFile, "%s%s", NumberString( LsbText, (long int)( Value & NibbleMask ), 16, 1 ),
					 NumberString( MsbText, (long int)( ( Value >> 4 ) & NibbleMask ), 16, 1 ) ) <= 0 )
			RunError( True, FileWriteError );

	} else if( fprintf( OutFile, "%s", NumberString( MsbText, (long int)Value, 16, 2 ) ) <= 0 )
		RunError( True, FileWriteError );
	...
	if( *HpAscCond ) {
		...
		if( *HpHeadCond ) {
			for( Count = 0; Count < 4; Count++ )
				if( fprintf( OutFile, "%s", NumberString( HexText, ( Check >> ( 4 * Count ) ) & NibbleMask, 16, 1 ) ) <=
					0 )
					RunError( True, FileWriteError );
...
{
	for( unsigned int idx = 0x0; idx < 0x100; idx++ ) Operators[idx] = False;
