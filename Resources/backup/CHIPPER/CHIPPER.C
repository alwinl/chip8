...
static void ListInstruction( long int Address, unsigned int Count, InstPointer Inst )
{
    ...
    if( fprintf( ListFile, "%s\n%s(%u).. %s: ", Inst->Loc.Text, Inst->Loc.Name, Inst->Loc.Line, NumberString( HexText, Address, 16, 3 ) ) <= 0 )
...
static void DecodeDirective( unsigned int Token, char **Line, InstPointer *Head, SymbolPointer Dir, SymbolPointer *Symb, SymbolPointer *Cond )
{
    ParamPointer Params;
    InstPointer Inst;
    unsigned int Count;
                ...
                if( ParamCheck( Count, 1, ListLength ) )
                    if( ResolveOption( Params->Param, UsedYesCond, UsedOnCond ) ) {
                        ...
                        for( Point = Params; Point; Point = Point->Next )
                            ...
                            } else if( !ResolveSymbol( Point->Param, &Value, *Symb,
                                                       &UsedLocation ) )
                                if( !Value )
                                    RunError( False, NoSymbolWarning );
                    } else if( Count > 1 )
...
static void EncodeValToken( InstPointer Inst, long int OpCode, long int Limit, SymbolPointer Symb, long int *Ver8, long int *Ver11 )
{
    long int Value;
...
static void EncodeRegRegOrValToken( InstPointer Inst, long int OpCode1, long int OpCode2, SymbolPointer Reg, SymbolPointer Symb )
{
    long int RegX;
    long int RegY;
    long int Value;
...
static void WriteCheckByte( long int *Check, unsigned char Value )
{
    ...
    if( *HpHeadCond ) {
        if( fprintf( OutFile, "%s%s", NumberString( LsbText, ( long int )( Value & NibbleMask ), 16, 1 ), NumberString( MsbText, ( long int )( ( Value >> 4 ) & NibbleMask ), 16, 1 ) ) <= 0 )
            RunError( True, FileWriteError );
    ...
    if( *HpAscCond ) {
        ...
        if( *HpHeadCond ) {
            for( Count = 0; Count < 4; Count++ )
                if( fprintf( OutFile, "%s", NumberString( HexText, ( Check >> ( 4 * Count ) ) & NibbleMask, 16, 1 ) ) <= 0 )
                    RunError( True, FileWriteError );
